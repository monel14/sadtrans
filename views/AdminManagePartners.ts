...
async function refreshPartnerView() {
    ...
    const contentArea = currentContainer.querySelector('.grid') || currentContainer.querySelector('p');
    if (contentArea) {
        contentArea.replaceWith(loadingIndicator);
    }
    ...
    try {
        ...
        if (allPartners.length === 0) {
            loadingIndicator.outerHTML = `<p class="text-center text-slate-500 p-4">Aucun partenaire trouvé.</p>`;
        } else {
        ...
        loadingIndicator.innerHTML = '<p class="text-center text-red-500 p-4">Erreur lors du chargement des données</p>';
...
export async function renderAdminManagePartnersView(): Promise<HTMLElement> {
    ...
    container.innerHTML = `
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
            ...
            <button id="create-partner-btn" class="btn btn-success w-full md:w-auto">
                <i class="fas fa-building mr-2"></i>Créer un Partenaire
            </button>
        </div>
    `;
    ...
    wrapperCard.addEventListener('beforeunload', () => {
        ...
        document.body.removeEventListener('agencyBalanceUpdated', refreshEventHandler);
